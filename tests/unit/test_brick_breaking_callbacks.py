...
class TestBrickBreakingCallbacks:
    ...
    def test_brick_breaking_callback_system(self):
        ...
        initial_bricks = [
            obj
            for obj in scene.level_data.get_objects_by_type("brick")
            if obj.is_active()
        ]
        ...
        breakable_brick = next(
            (obj for obj in initial_bricks if "breakable" in obj.id), None
        )
        ...
        print(
            f"Positioned player at: {scene.player_rect}, velocity_y={scene.player_velocity_y}"
        )
        ...
        ), f"Expected some bricks to be broken, but count went from {initial_count} to {final_count}"
        assert (
            len(broken_bricks) > 0
        ), "Expected OnBreak callbacks to be triggered, but none were called"
        ...
        for brick_id in broken_bricks:
            ...
            assert (
                not brick.is_active()
            ), f"Brick {brick_id} should be inactive after breaking"
    ...
    def test_object_callback_registration(self):
        ...
        assert callback_args == (
            ("test_arg",),
            {"key": "test_value"},
        ), f"Callback args mismatch: {callback_args}"

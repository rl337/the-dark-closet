...
class TestCharacterWalkCycleSequence:
    ...
    def test_walk_cycle_sequence(self):
        ...
        for phase_name, phase_duration in sequence:
            ...
            for frame in range(phase_duration):
                ...
                if frame_count % 10 == 0:
                    ...
                    filename = f"walk_cycle_{frame_count:03d}_{phase_name}_{frame:02d}.png"
        ...
        assert abs(character.x - 400) < 10, f"Character should be near starting position, got {character.x}"
    ...
    def test_walk_cycle_asset_generation(self):
        ...
        assert len(left_walk_assets) == 4, f"Should have 4 left walk frames, got {len(left_walk_assets)}"
        assert len(right_walk_assets) == 4, f"Should have 4 right walk frames, got {len(right_walk_assets)}"
    ...
    def test_character_animation_timing(self):
        ...
        assert character.animation_frame == (initial_frame + 1) % character.walk_cycle_frames
        
        # Update with half animation frame duration
        character.update(character.animation_speed / 2)
        assert character.animation_frame == (initial_frame + 1) % character.walk_cycle_frames  # Should not advance
        
        # Update with another half to complete the frame
        character.update(character.animation_speed / 2)
        assert character.animation_frame == (initial_frame + 2) % character.walk_cycle_frames

...
class CharacterRenderingTester:
    ...
    def _detect_character_pixels(self, surface_array: np.ndarray) -> bool:
        ...
        character_region = surface_array[
            center_y - 60 : center_y + 60, center_x - 60 : center_x + 60
        ]
        ...
        for y in range(character_region.shape[0]):
            for x in range(character_region.shape[1]):
                ...
                if not np.allclose(pixel, sky_color, atol=20) and not np.allclose(
                    pixel, brick_color, atol=20
                ):
    ...
    def _detect_vertical_lines(self, surface_array: np.ndarray) -> bool:
        ...
        return (
            vertical_lines > width * 0.1
        )  # More than 10% of columns are vertical lines

    def _get_dominant_colors(
        self, surface_array: np.ndarray
    ...
    def test_character_rendering_has_proper_sprites(self, character_tester):
        ...
        for screenshot_path in screenshots:
            ...
            if "error" in analysis:
                issues_found.append(
                    f"Analysis error in {screenshot_path.name}: {analysis['error']}"
                )
                continue
            ...
            if analysis["has_hud_text"]:
                issues_found.append(
                    f"HUD text detected in {screenshot_path.name} - should be minimal or absent"
                )
            ...
            if analysis["pixel_variance"] < 1000:
                issues_found.append(
                    f"Low pixel variance in {screenshot_path.name} - image may be too simple"
                )
        ...
        assert (
            len(issues_found) == 0
        ), f"Character rendering issues found: {issues_found}"
    ...
    def test_character_animation_sequences(self, character_tester):
        ...
        for screenshot_path in screenshots:
            ...
            if "error" not in analysis:
                ...
                sequence_name = screenshot_path.stem.split("_")[
                    -2
                ]  # e.g., "idle", "walk", "jump"
        ...
        for sequence_name, analyses in sequence_analyses.items():
            ...
            frames_with_character = sum(
                1 for analysis in analyses if analysis["has_character_pixels"]
            )
            character_coverage = frames_with_character / len(analyses)

            print(
                f"  Character coverage: {character_coverage:.2f} ({frames_with_character}/{len(analyses)} frames)"
            )

            if (
                character_coverage < 0.8
            ):  # Less than 80% of frames have character pixels
                issues_found.append(
                    f"{sequence_name} sequence has poor character coverage: {character_coverage:.2f}"
                )
            ...
            if variance_std > 5000:  # High variation in detail
                issues_found.append(
                    f"{sequence_name} sequence has inconsistent rendering: {variance_std:.2f}"
                )
        ...
        assert (
            len(issues_found) == 0
        ), f"Character animation issues found: {issues_found}"

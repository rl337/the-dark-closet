...
class TestCharacterShowcase:
    ...
    def test_character_asset_generation_showcase(self):
        ...
        # Categorize assets
        basic_assets = [k for k in asset_paths.keys() if not any(x in k for x in ["_left", "_right", "_forward", "_back", "walk_"])]
        directional_assets = [k for k in asset_paths.keys() if any(x in k for x in ["_left", "_right", "_forward", "_back"]) and not k.startswith("walk_")]

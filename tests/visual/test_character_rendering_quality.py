...
def test_character_rendering_quality():
    ...
    assert (
        character_pixels > 500
    ), f"Character region should have significant non-background pixels, found {character_pixels}"
    assert (
        unique_colors > 5
    ), f"Character should have multiple colors, found {unique_colors}"
    assert (
        edge_ratio > 0.5
    ), f"Character should not be dominated by vertical lines, edge ratio: {edge_ratio:.2f}"
...
def test_level_tile_rendering():
    ...
    assert (
        brick_pixels > 1000
    ), f"Should have significant brick pixels, found {brick_pixels}"
    assert (
        line_ratio > 0.3
    ), f"Tiles should not be dominated by vertical lines, ratio: {line_ratio:.2f}"
    ...
    assert (
        unique_colors > 5
    ), f"Tiles should have color variety, found {unique_colors} colors"
...
def test_overall_visual_composition():
    ...
    assert (
        unique_colors > 10
    ), f"Should have multiple colors for good composition, found {unique_colors}"
    assert contrast > 40, f"Should have reasonable contrast, found {contrast:.2f}"
    assert (
        avg_local_variance > 100
    ), f"Should have visual interest, local variance: {avg_local_variance:.2f}"

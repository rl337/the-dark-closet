...
class CharacterRenderingValidator:
    ...
    def capture_clean_scene(
        self, name: str, level_path: Path, actions: List[Tuple[str, int, int]], hide_hud: bool = True
    ) -> List[Path]:
        ...
        for description, keys, frames in actions:
            for frame in range(frames):
                ...
                if hide_hud:
                    ...
                    screenshot_path = (
                        self.output_dir / f"{name}_{description}_{frame:02d}_with_hud.png"
                    )
    ...
    def validate_character_rendering(self, screenshot_path: Path) -> dict:
        ...
        try:
            ...
            if validation["character_quality"] < 0.05:  # Very low threshold for now
                validation["issues"].append(f"Poor character quality: {validation['character_quality']:.2f}")
                validation["valid"] = False
            
            if validation["composition"] < 0.3:  # Lowered threshold
                validation["issues"].append(f"Poor composition: {validation['composition']:.2f}")
                validation["valid"] = False
...
class TestCharacterRenderingFixes:
    ...
    def test_character_rendering_clean_view(self, character_validator):
        ...
        for screenshot_path in screenshots:
            validation = character_validator.validate_character_rendering(screenshot_path)
        ...
        assert all_valid, "Character rendering validation failed - check screenshots in build/character_rendering_validation/"
    ...
    def test_character_animation_quality(self, character_validator):
        ...
        for sequence_name, validations in sequence_validations.items():
            ...
            print(f"  Validity rate: {validity_rate:.2f} ({valid_frames}/{len(validations)} frames)")
            ...
            qualities = [v['character_quality'] for v in validations if 'character_quality' in v]
            if qualities:
    ...
    def test_character_rendering_with_hud_comparison(self, character_validator):
        ...
        for clean_path, hud_path in zip(clean_screenshots, hud_screenshots):
            clean_validation = character_validator.validate_character_rendering(clean_path)
        ...
        assert all(v['valid'] for v in [character_validator.validate_character_rendering(p) for p in clean_screenshots]), "Clean version should be valid"

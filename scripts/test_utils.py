...
def assert_center_mass_at(
    ...
    if actual_pos is None:
        print("ERROR: Center mass dot not found in surface")
        return False
    ...
    if dx <= tolerance and dy <= tolerance:
        print(
            f"✓ Center mass at ({actual_x}, {actual_y}) - within tolerance of ({expected_x}, {expected_y})"
        )
        return True
    else:
        print(
            f"✗ Center mass at ({actual_x}, {actual_y}) - expected ({expected_x}, {expected_y}) ±{tolerance}"
        )
        return False
...
def run_action_with_assertions(
    ...
    if initial_pos is None:
        print("ERROR: Could not find initial center mass position")
        return False
...
def test_move_left_right(out_dir: Path) -> bool:
    ...
    action_sequence = [
        (
            ...
            {pygame.K_RIGHT},
            4,
            (spawn_px[0] + 52 + 12, spawn_px[1] + 60 + 4),
        ),  # Move right 4 frames
        (
            "move_left",
            {pygame.K_LEFT},
            4,
            (spawn_px[0] + 52, spawn_px[1] + 60 + 8),
        ),  # Move left 4 frames
    ]

    return run_action_with_assertions(
        "move_left_right", room, spawn_px, action_sequence, out_dir
    )
...
def test_jump(out_dir: Path) -> bool:
    ...
    action_sequence = [
        (
            ...
            {pygame.K_SPACE},
            4,
            (spawn_px[0] + 52, spawn_px[1] + 60 + 4),
        ),  # Try to jump (will fall)
        (
            "continue_falling",
            None,
            4,
...
def test_break_brick(out_dir: Path) -> bool:
    ...
    action_sequence = [
        (
            ...
            {pygame.K_RIGHT},
            2,
            (spawn_px[0] + 52 + 6, spawn_px[1] + 60 + 2),
        ),  # Move right 2 frames
        (
            "break_brick",
            {pygame.K_SPACE},
            4,
            (spawn_px[0] + 52 + 6, spawn_px[1] + 60 + 6),
        ),  # Try to break (will fall)
        (
            "land_after_break",
            None,
            2,
            (spawn_px[0] + 52 + 6, spawn_px[1] + 60 + 8),
        ),  # Continue falling
    ]

    return run_action_with_assertions(
        "break_brick", room, spawn_px, action_sequence, out_dir
    )
...
def test_ladders(out_dir: Path) -> bool:
    ...
    return run_action_with_assertions(
        "ladders", room, spawn_px, action_sequence, out_dir
    )

...
class PinocchioAssetGenerator:
    ...
    def draw_circle_with_shading(self, surface: pygame.Surface, center: Tuple[int, int], 
    ...
    def draw_ellipse_with_shading(self, surface: pygame.Surface, rect: pygame.Rect, 
        ...
        # Highlight
        highlight_rect = pygame.Rect(rect.x + rect.width // 4, rect.y + rect.height // 4,
    ...
    def generate_head(self) -> pygame.Surface:
        ...
        # Head (wood-like)
        self.draw_circle_with_shading(surface, center, radius, self.colors['wood'], self.colors['wood_dark'])
    ...
    def generate_face(self, surface: pygame.Surface, head_center: Tuple[int, int]) -> None:
        ...
        for i in range(3):
            line_y = y - 20 + i * 15
            pygame.draw.line(surface, self.colors['wood_dark'], 
                           (x - 25, line_y), (x + 25, line_y), 1)
    ...
    def generate_torso(self) -> pygame.Surface:
        ...
        self.draw_ellipse_with_shading(surface, torso_rect, self.colors['wood'], self.colors['wood_dark'])
        ...
        for i in range(4):
            ...
            pygame.draw.line(surface, self.colors['wood_dark'], 
                           (center_x - 25, line_y), (center_x + 25, line_y), 1)
    ...
    def generate_left_arm(self) -> pygame.Surface:
        ...
        self.draw_ellipse_with_shading(surface, upper_arm_rect, self.colors['wood'], self.colors['wood_dark'])
        
        # Lower arm
        lower_arm_rect = pygame.Rect(center_x - 8, center_y + 10, 16, 30)
        self.draw_ellipse_with_shading(surface, lower_arm_rect, self.colors['wood_light'], self.colors['wood_dark'])
    ...
    def generate_left_leg(self) -> pygame.Surface:
        ...
        self.draw_ellipse_with_shading(surface, upper_leg_rect, self.colors['pants'], self.colors['wood_dark'])
        
        # Lower leg
        lower_leg_rect = pygame.Rect(center_x - 10, center_y + 10, 20, 40)
        self.draw_ellipse_with_shading(surface, lower_leg_rect, self.colors['pants'], self.colors['wood_dark'])
    ...
    def generate_eyes_closed(self) -> pygame.Surface:
        ...
        pygame.draw.arc(surface, self.colors['eyes'], 
                       pygame.Rect(center_x - 21, center_y - 6, 12, 12), 0, math.pi, 2)
        pygame.draw.arc(surface, self.colors['eyes'], 
                       pygame.Rect(center_x + 9, center_y - 6, 12, 12), 0, math.pi, 2)
    ...
    def generate_mouth_neutral(self) -> pygame.Surface:
        ...
        pygame.draw.line(surface, self.colors['mouth'], 
    ...
    def generate_all_assets(self, output_dir: Path) -> Dict[str, str]:
        ...
        # Body parts
        assets['head'] = self._save_asset(self.generate_head(), body_dir / "head.png")
        assets['torso'] = self._save_asset(self.generate_torso(), body_dir / "torso.png")
        assets['left_arm'] = self._save_asset(self.generate_left_arm(), body_dir / "left_arm.png")
        assets['right_arm'] = self._save_asset(self.generate_right_arm(), body_dir / "right_arm.png")
        assets['left_leg'] = self._save_asset(self.generate_left_leg(), body_dir / "left_leg.png")
        assets['right_leg'] = self._save_asset(self.generate_right_leg(), body_dir / "right_leg.png")
        
        # Facial features
        assets['eyes_open'] = self._save_asset(self.generate_eyes_open(), face_dir / "eyes_open.png")
        assets['eyes_closed'] = self._save_asset(self.generate_eyes_closed(), face_dir / "eyes_closed.png")
        assets['mouth_neutral'] = self._save_asset(self.generate_mouth_neutral(), mouth_dir / "mouth_neutral.png")
        assets['mouth_open'] = self._save_asset(self.generate_mouth_open(), mouth_dir / "mouth_open.png")
        
        # Gear
        assets['hat'] = self._save_asset(self.generate_hat(), gear_dir / "hat.png")
        
        return assets
    
    def _save_asset(self, surface: pygame.Surface, path: Path) -> str:
        """Save a surface as a PNG file."""
        pygame.image.save(surface, str(path))
        return str(path)


def generate_character_assets(output_dir: Path = Path("generated_assets")) -> Dict[str, str]:
    """Generate all Pinocchio-inspired character assets."""
    generator = PinocchioAssetGenerator()
    return generator.generate_all_assets(output_dir)
